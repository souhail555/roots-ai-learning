...
COMMENT ON COLUMN sessions.owner_id IS
  'Supabase auth subject (auth.uid()) that owns this assessment. Written by the server on session creation; never user-supplied.';
...
USING (
  owner_id = auth.uid()
  OR (owner_id IS NULL AND email = auth.jwt() ->> 'email')
