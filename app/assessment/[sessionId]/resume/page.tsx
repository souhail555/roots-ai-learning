...
export default function ResumePage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const nextModule = assessmentModules.find((module) => !session.completedModules.includes(module.id)) ?? assessmentModules.at(-1)!;
  return <main className="route-shell"><p className="eyebrow">ROOTS / RESUME</p><h1>Welcome back</h1><p className="route-lede">You have completed {session.completedModules.length} of {assessmentModules.length} modules. Continue from {nextModule.title}.</p><p className="save-meta">Last saved: {session.updatedAt ? new Date(session.updatedAt).toLocaleString() : "Not available"}</p><Link className="primary-button" href={`/assessment/${sessionId}/module/${nextModule.id}`}>Resume Assessment</Link></main>;
}